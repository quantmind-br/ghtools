...
VERBOSE=false
QUIET=false
YES_MODE=false   # Non-interactive mode (auto-confirm with defaults)
...
gum_confirm() {
  ...
  if [[ "$YES_MODE" == "true" ]] || [[ "${GHTOOLS_TEST_MODE:-}" == "1" ]]; then
...
gum_input() {
  ...
  # Non-interactive mode: return default or placeholder
  if [[ "$YES_MODE" == "true" ]] || [[ "${GHTOOLS_TEST_MODE:-}" == "1" ]]; then
  ...
  if use_gum; then
    ...
    local response
    read -rp "$prompt$placeholder: " response
...
gum_filter() {
  ...
  # Non-interactive mode: return all/first line from stdin
  if [[ "$YES_MODE" == "true" ]] || [[ "${GHTOOLS_TEST_MODE:-}" == "1" ]]; then
    if [[ "$multi" == "true" ]]; then
      cat
    else
...
gum_write() {
  ...
  # Non-interactive mode: return placeholder or read from stdin if available
  if [[ "$YES_MODE" == "true" ]] || [[ "${GHTOOLS_TEST_MODE:-}" == "1" ]]; then
    if [[ ! -t 0 ]]; then
      cat
    else
...
${YELLOW}OPTIONS:${NC}
    ...
    -y, --yes           Non-interactive mode (auto-confirm with defaults)
...
# Safe read with test mode support
# Usage: safe_read "prompt" "default_value" variable_name
safe_read() {
  local prompt="$1"
  local default="${2:-}"
...
check_delete_scope() {
  if ! gh auth status 2>&1 | grep -q "delete_repo"; then
    ...
    echo ""
    local response
    safe_read "Would you like to refresh auth with delete_repo scope? [y/N]: " "n" response
    if [[ "$response" =~ ^[Yy]$ ]]; then
...
action_delete() {
  ...
  local confirm
  safe_read "Type 'DELETE' to confirm, or anything else for Dry Run: " "DELETE" confirm
  local dry_run=true
...
action_create() {
  ...
  # Get repository name
  local name
  name=$(gum_input "Repository name" "› " "")
  ...
  # Get description
  local desc
  desc=$(gum_input "Description (optional)" "› " "")

  # Select visibility
  local vis
  vis=$(gum_choose "Visibility:" "public" "private")
  [ -z "$vis" ] && vis="private"

  # Select template
  local tpl
  tpl=$(gum_choose "Template:" "none" "python" "node" "go")
...
action_fork() {
  ...
  if [ -z "$search_query" ]; then
    search_query=$(gum_input "Search for repository to fork" "› " "")
    if [ -z "$search_query" ]; then
...
action_explore() {
  ...
  if [ -z "$search_query" ]; then
    ...
    # Get search query
    search_query=$(gum_input "Search query (e.g., 'machine learning')" "› " "")
    ...
    # Get sort option
    local sort_choice
    sort_choice=$(gum_choose "Sort by:" "stars" "forks" "updated" "help-wanted-issues")
    sort_by="${sort_choice:-stars}"

    # Get language filter
    language=$(gum_input "Filter by language (leave empty for all)" "› " "")
  fi
  ...
  local action
  action=$(gum_choose "Select action:" \
...
action_archive() {
  ...
  if ! gum_confirm "Continue?" "no"; then
    print_info "Cancelled."
    return
  fi
...
action_search() {
  ...
  d|"Delete"*)
    ...
    local confirm
    confirm=$(gum_input "Type DELETE to confirm" "› " "DELETE")
    if [ "$confirm" = "DELETE" ]; then
      for repo in "${repos[@]}"; do
...
action_pr_create() {
  ...
  local title
  title=$(gum_input "PR Title" "› " "$current_branch")
  [ -z "$title" ] && title="$current_branch"

  local draft=""
  if gum_confirm "Create as draft?" "no"; then
    draft="--draft"
  fi
...
main() {
  ...
  if [ $# -eq 0 ]; then
    # In test mode, just show menu once
    if [[ "$YES_MODE" == "true" ]] || [[ "${GHTOOLS_TEST_MODE:-}" == "1" ]]; then
      show_menu
    else
      while true; do
        show_menu
        echo ""
        gum_confirm "Continue?" "yes" || exit 0
      done
    fi
  fi
